...
u_bar_min = zeros(8,1); % lower bound for u_bar
k = 1e-5; % thrust coefficient
omega_max = 2*pi*8000/60; % 8000 rpm into rad/s
u_bar_max = omega_max^2 + u_bar_min; % upper bound for u_bar
...
for col_loss = 1:m % between 1 and m
    ...
    [time_unperturbed, u_bar_p] = solution_unperturbed(B_bar, u_bar_min, u_bar_max, C);
    [time_perturbed, u_p, w_p] = solution_perturbed(B_bar, u_bar_min, u_bar_max, C, col_loss);
    if time_unperturbed == Inf
        time_ratio(1,col_loss) = Inf;
    else
        time_ratio(1,col_loss) = time_perturbed/time_unperturbed;
    end
    [time_unperturbed, u_bar_m] = solution_unperturbed(B_bar, u_bar_min, u_bar_max, -C);
    [time_perturbed, u_m, w_m] = solution_perturbed(B_bar, u_bar_min, u_bar_max, -C, col_loss);
    if time_unperturbed == Inf
